...
namespace Netezos.Rpc.Queries
{
    ...
    public class BlocksQuery : RpcObject
    {
        ...
        /// <returns></returns>
        public async Task<JToken> GetAsync(int length)
            => await Client.GetJson($"{Query}?length={length}");
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<JToken> GetAsync(DateTime minDate, int length = 1)
            => await Client.GetJson($"{Query}?min_date={minDate.ToEpoch()}&length={length}");
        ...
        /// <returns></returns>
        public async Task<JToken> GetAsync(string head, int length = 1)
            => await Client.GetJson($"{Query}?head={head}&length={length}");
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<JToken> GetAsync(List<string> heads, int length = 1)
            => await Client.GetJson($"{Query}?{String.Join("&", heads.Select(h => $"head={h}"))}&length={length}");
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<T> GetAsync<T>(int length)
            => await Client.GetJson<T>($"{Query}?length={length}");
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<T> GetAsync<T>(DateTime minDate, int length = 1)
            => await Client.GetJson<T>($"{Query}?min_date={minDate.ToEpoch()}&length={length}");
        ...
        /// <param name="length">Number of blocks (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<T> GetAsync<T>(string head, int length = 1)
            => await Client.GetJson<T>($"{Query}?head={head}&length={length}");
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public async Task<T> GetAsync<T>(List<string> heads, int length = 1)
            => await Client.GetJson<T>($"{Query}?{String.Join("&", heads.Select(h => $"head={h}"))}&length={length}");
    }
}
