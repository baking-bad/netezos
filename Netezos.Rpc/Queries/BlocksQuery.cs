...
namespace Netezos.Rpc.Queries
{
    ...
    public class BlocksQuery : RpcObject
    {
        ...
        /// <returns></returns>
        public Task<JToken> GetAsync(int length)
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public Task<JToken> GetAsync(DateTime minDate, int length = 1)
            => Client.GetJson($"{Query}?min_date={minDate.ToEpoch()}&length={length}");
        ...
        /// <returns></returns>
        public Task<JToken> GetAsync(string head, int length = 1)
        ...
        /// <param name="length">Number of blocks per head (head + predessors) to returns</param>
        /// <returns></returns>
        public Task<JToken> GetAsync(List<string> heads, int length = 1)
        ...
        public Task<T> GetAsync<T>(DateTime minDate, int length = 1)
            => Client.GetJson<T>($"{Query}?min_date={minDate.ToEpoch()}&length={length}");
