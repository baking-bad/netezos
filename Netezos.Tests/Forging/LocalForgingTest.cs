...
namespace Netezos.Tests.Forging
{
    public class LocalForgingTest
    {
        ...
        {
            //"../../../Forging/operations\\onmRCWyVwCnTgsiYGGU6xUAFtuybv93d92DKxcHpc22gFJaPHK9/forged.hex" // UnforgeGroupAsync candidate
            //string opHex = File.ReadAllText(@"../../../Forging/operations/onik4HScQRGwsNys3n1HNs2ZuESfkui5Sib7JgFqGUkcTbv4rZR/forged.hex");
            //byte[] op = Hex.Parse(opHex);
            ...
            foreach (var directory in directories)
            {
                ...
                switch ((int)json.contents.count)
                {
                    ...
                    case int n when (n == 1):
                        ...
                        var opByte = Hex.Parse(File.ReadAllText($"{directory}/forged.hex"));
                        var op = await localForge.UnforgeOperationAsync(opByte);
                        var serialized = JsonSerializer.Serialize(op.Item2, op.Item2.GetType(), options);
                        var deserialized = DJson.Parse(serialized);

                        Assert.Equal(json.branch, op.Item1);
                        bool equal = jsonComparer.Equals((JsonElement)json.contents[0], (JsonElement)deserialized);
                        Assert.True(equal);
                        //Assert.True(opByte == Hex.Convert(localByte), $"{directory}");

                        //var serialized = JsonSerializer.Serialize(oc, options);
                        //var deserialized = DJson.Parse(serialized, options);
                        break;
                    default:
                        throw new ArgumentException();
                }
