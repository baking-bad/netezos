...
namespace Netezos.Rpc.Queries.Post
{
    ...
    public class InjectBlockQuery : RpcMethod
    {
        ...
        /// <returns></returns>
        public Task<dynamic> PostAsync(object data, List<List<object>> operations,bool async = false, bool force = false, Chain chain = Chain.Main)
            => Client.PostJson(
        ...
        /// <returns></returns>
        public Task<T> PostAsync<T>(object data, List<List<object>> operations, bool async = false, bool force = false, Chain chain = Chain.Main)
            => Client.PostJson<T>(
                $"{Query}?async={async}&force={force}&chain={chain.ToString().ToLower()}",
                new { data, operations });
    }
