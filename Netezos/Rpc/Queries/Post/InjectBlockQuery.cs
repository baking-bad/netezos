...
namespace Netezos.Rpc.Queries.Post
{
    ...
    public class InjectBlockQuery : RpcMethod
    {
        ...
        /// <returns></returns>
        public Task<dynamic> PostAsync(string data, IEnumerable<IEnumerable<object>> operations, bool async = false, bool force = false, string chain = "main")
            => Client.PostJson(
        ...
        /// <returns></returns>
        public Task<T> PostAsync<T>(string data, IEnumerable<IEnumerable<object>> operations, bool async = false, bool force = false, string chain = "main")
            => Client.PostJson<T>(
                $"{Query}?async={async}&force={force}&chain={chain}",
                new { data, operations });
    }
